...
class Font {
 public:
  ...
  void GetPixels(char32_t c, int bytesPerPixel, int xsize, uint8_t *data,
                 int &width, uint8_t *foreColor, uint8_t *backColor);
  ...
  uint32_t GetIndex(char32_t c) const { return _chix.GetIndex(c); }
  ...
  uint16_t GetAsce() const { return _asce.Get(); }
  uint16_t GetDesc() const { return _desc.Get(); }
 ...
 private:
  ...
  class Maxw : public Section {
   ...
   private:
    virtual const char *GetSignature() override { return "MAXW"; }
  ...
  class Maxh : public Section {
   ...
   private:
    virtual const char *GetSignature() override { return "MAXH"; }
    virtual void LoadSub(char *buf, uint32_t length) override {
      assert(length == 2);
      _value = be16toh(*(reinterpret_cast<uint16_t *>(buf)));
    }
  ...
  class Chix : public Section {
   ...
   private:
    virtual const char *GetSignature() override { return "CHIX"; }
    virtual void LoadSub(char *buf, uint32_t length) override {
      _len = length;
      _buf = buf;
    }
  ...
  class Asce : public Section {
   ...
   private:
    virtual const char *GetSignature() override { return "ASCE"; }
    virtual void LoadSub(char *buf, uint32_t length) override {
      assert(length == 2);
      _value = be16toh(*(reinterpret_cast<uint16_t *>(buf)));
    }
  ...
  class Desc : public Section {
   ...
   private:
    virtual const char *GetSignature() override { return "DESC"; }
    virtual void LoadSub(char *buf, uint32_t length) override {
      assert(length == 2);
      _value = be16toh(*(reinterpret_cast<uint16_t *>(buf)));
    }
  ...
  class Data : public Section {
   public:
    ...
    void SetBuffer(char *buf) { _buf = buf; }

   private:
    virtual const char *GetSignature() override { return "DATA"; }
    virtual void LoadSub(char *buf, uint32_t length) override {
