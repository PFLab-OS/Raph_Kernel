...
void ApicCtrl::Setup() {
  ...
  for (uint32_t offset = 0; offset < _madt->header.Length - sizeof(MADT);) {
    ...
    switch (madtSt->type) {
      ...
      case MADTStType::kIOAPIC: {
        _ioapic._controller[ioapic_num].Setup(
            reinterpret_cast<MADTStIOAPIC *>(ptr));
        ioapic_num++;
      } break;
      default:
        break;
...
void ApicCtrl::StartAPs() {
  ...
  for (int i = 0; i < _lapic->_ncpu; i++) {
    ...
    *stack_of_others = KernelStackCtrl::GetCtrl().AllocThreadStack(cpuid);
    _lapic->Start(_lapic->_apic_info[i].id,
                  reinterpret_cast<uint64_t>(entryothers));
...
void ApicCtrl::Lapic::Setup() {
  ...
  // WriteReg(RegisterOffset::kLvtLint0, kDeliverModeFixed |
  // kDeliverModeExtint); WriteReg(RegisterOffset::kLvtLint1, kDeliverModeFixed
  // | kDeliverModeNmi);
  WriteReg(RegisterOffset::kLvtErr,
           kDeliverModeFixed | Idt::ReservedIntVector::kError);

  kassert(idt != nullptr);
  idt->SetExceptionCallback(cpu_ctrl->GetCpuId(), Idt::ReservedIntVector::kIpi,
                            IpiCallback, nullptr, Idt::EoiType::kLapic);
  idt->SetExceptionCallback(cpu_ctrl->GetCpuId(),
                            Idt::ReservedIntVector::k8259Spurious1,
                            PicSpuriousCallback, nullptr, Idt::EoiType::kNone);
  idt->SetExceptionCallback(cpu_ctrl->GetCpuId(),
                            Idt::ReservedIntVector::k8259Spurious2,
                            PicSpuriousCallback, nullptr, Idt::EoiType::kNone);
  idt->SetExceptionCallback(cpu_ctrl->GetCpuId(),
                            Idt::ReservedIntVector::k8259Rtc,
                            PicUnknownCallback, nullptr, Idt::EoiType::kNone);

  WriteReg(RegisterOffset::kSvr,
           kRegSvrApicEnableFlag | Idt::ReservedIntVector::kError);
}
...
void ApicCtrl::Lapic::Start(uint32_t apicId, uint64_t entryPoint) {
  ...
  WriteIcr(apicId,
           kDeliverModeInit | kRegIcrTriggerModeLevel | kRegIcrLevelAssert);
  timer->BusyUwait(200);
...
void ApicCtrl::Lapic::SetupTimer(int interval) {
  ...
  kassert(idt != nullptr);
  int vector = idt->SetIntCallback(cpu_ctrl->GetCpuId(), TmrCallback, nullptr,
                                   Idt::EoiType::kLapic);
...
void ApicCtrl::Lapic::SendIpi(uint32_t destid) {
  WriteIcr(destid, kDeliverModeFixed | kRegIcrTriggerModeLevel |
                       kRegIcrDestShorthandNoShortHand |
                       Idt::ReservedIntVector::kIpi);
}
...
int ApicCtrl::Ioapic::GetControllerIndexFromIrq(uint32_t irq) {
  ...
  for (int j = 0; j < _controller_num; j++) {
    if (_controller[j].int_base <= irq &&
        irq <= _controller[j].int_base + _controller[j].int_max) {
      controller_index = j;
      break;
    }
  }
...
bool ApicCtrl::Ioapic::Controller::SetupInt(uint32_t irq, uint8_t lapicid,
                                            uint8_t vector, bool trigger_mode,
                                            bool polarity) {
  ...
  Write(kRegRedTbl + 2 * irq,
        kRegRedTblFlagValueDeliveryFixed | kRegRedTblFlagDestModePhys |
            (trigger_mode ? kRegRedTblFlagTriggerModeEdge
                          : kRegRedTblFlagTriggerModeLevel) |
            (polarity ? kRegRedTblFlagPolarityHighActive
                      : kRegRedTblFlagPolarityLowActive) |
            vector);
  return true;
}
