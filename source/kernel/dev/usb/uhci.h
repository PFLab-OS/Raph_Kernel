...
class DevUhci final : public DevPci {
 public:
  DevUhci(uint8_t bus, uint8_t device, uint8_t function)
 ...
 private:
  ...
  class TransferDescriptor {
   public:
    ...
    static PacketIdentification ConvertPacketIdentification(
        UsbCtrl::PacketIdentification pid) {
      switch (pid) {
    ...
    void SetStatus(bool interrupt, bool isochronous, bool low_speed,
                   bool short_packet) {
      _status = (short_packet ? (1 << 29) : 0) | (low_speed ? (1 << 26) : 0) |
                (isochronous ? (1 << 25) : 0) | (interrupt ? (1 << 24) : 0) |
                (1 << 23);
    }
    ...
    // for debug
    uint32_t GetStatus() { return __sync_fetch_and_or(&_status, 0); }
    void SetToken(UsbCtrl::PacketIdentification pid, int device_address,
                  int endpoint, bool data_toggle, int max_len) {
      ...
      _token = (max_len << 21) | (data_toggle ? (1 << 19) : 0) |
               (endpoint << 15) | (device_address << 8) |
               static_cast<uint8_t>(ConvertPacketIdentification(pid));
    }
    void SetBuffer(virt_addr buf, int offset) {
      SetBufferSub(k2p(buf + offset));
    }
    void SetBuffer(/* nullptr */) { SetBufferSub(0); }
    void SetContainer(TdContainer &container) {
      SetStatus(container.interrupt_on_complete, container.isochronous,
                container.low_speed, container.short_packet);
      SetToken(container.pid, container.device_address, container.endpoint,
               container.data_toggle, container.total_bytes);
   ...
   private:
    void SetNextSub(phys_addr next, bool depth_first, bool q, bool terminate) {
      ...
      assert((next & 0xF) == 0);
      _next = (next & 0xFFFFFFF0) | (depth_first ? (1 << 2) : 0) |
              (q ? (1 << 1) : 0) | (terminate ? (1 << 0) : 0);
    }
  ...
  class QueueHead {
   public:
    ...
    void SetVerticalNext(/* nullptr */) { SetVerticalNextSub(0, false, true); }
    bool IsVerticalNextNull() { return (vertical_next & 1) != 0; }
   ...
   private:
    void SetHorizontalNextSub(phys_addr next, bool q, bool terminate) {
      ...
      assert((next & 0xF) == 0);
      horizontal_next =
          (next & 0xFFFFFFF0) | (q ? (1 << 1) : 0) | (terminate ? (1 << 0) : 0);
    }
    ...
    void SetHorizontalNext(uint32_t next) { horizontal_next = next; }
  ...
  class UhciManager : public DevUsbController::Manager {
   public:
    UhciManager(int num_td, QueueHead *qh,
  ...
  class FramePointer {
   public:
    ...
    void Set(QueueHead *next) { SetSub(v2p(ptr2virtaddr(next)), true); }
  ...
  template <class T>
  volatile T ReadControllerReg(uint16_t reg);

  template <class T>
  void WriteControllerReg(uint16_t reg, T data);
  ...
  class DevUhciUsbController : public DevUsbController {
   public:
    ...
    virtual bool SendControlTransfer(UsbCtrl::DeviceRequest *request,
      ...
      return _dev_uhci->SendControlTransfer(request, data, data_size,
                                            device_addr);
    }
    virtual sptr<DevUsbController::Manager> SetupInterruptTransfer(
      ...
      return _dev_uhci->SetupInterruptTransfer(
...
template <>
inline volatile uint16_t DevUhci::ReadControllerReg<uint16_t>(uint16_t reg) {
  return inw(_base_addr + reg);
}

template <>
inline void DevUhci::WriteControllerReg<uint16_t>(uint16_t reg, uint16_t data) {
  outw(_base_addr + reg, data);
}

template <>
inline void DevUhci::WriteControllerReg<uint32_t>(uint16_t reg, uint32_t data) {
...
inline void DevUhci::DisablePort(int port) {
  while (true) {
    WriteControllerReg<uint16_t>(
        kCtrlRegPortBase + port * 2,
        ReadControllerReg<uint16_t>(kCtrlRegPortBase + port * 2) &
            ~kCtrlRegPortFlagEnable);
    if ((ReadControllerReg<uint16_t>(kCtrlRegPortBase + port * 2) &
         kCtrlRegPortFlagEnable) == 0) {
      break;
    }
    asm volatile("" ::: "memory");
  }
