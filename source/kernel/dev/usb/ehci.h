 ...
 * Author: Liva
 *
 * reference: Enhanced Host Controller Interface Specification for Universal
 * Serial Bus
 *
 */
...
class DevEhci final : public DevPci {
 public:
  DevEhci(uint8_t bus, uint8_t device, uint8_t function)
      : DevPci(bus, device, function), _controller_dev(this) {}
 ...
 private:
  ...
  class FrameListElementPointer {
   public:
    void Set(/* nullptr */) { SetSub(0, 0, true); }
    void Set(QueueHead32 *ptr) { SetSub(v2p(ptr2virtaddr(ptr)), 1, false); }
    void Set(QueueHead64 *ptr) { SetSub(v2p(ptr2virtaddr(ptr)), 1, false); }
  ...
  class TransferDescriptor32 {
   public:
    ...
    void SetNext(/* nullptr */) { SetNextSub(0, true); }
    uint32_t GetToken() { return _token; }
    uint8_t GetStatus() { return _token & 0xFF; }
    bool IsActiveOfStatus() { return _token & (1 << 7); }
    bool IsDataBufferErrorOfStatus() { return _token & (1 << 5); }
    virt_addr GetBuffer() { return p2v(_buffer_pointer[0]); }
    void SetTokenAndBuffer(TdContainer &container) {
      SetTokenAndBuffer(container.interrupt_on_complete, container.data_toggle,
                        container.pid, container.total_bytes,
                        ptr2virtaddr(container.buf));
    }
    void SetTokenAndBuffer(bool interrupt_on_complete, bool data_toggle,
                           UsbCtrl::PacketIdentification pid, int total_bytes,
                           virt_addr buf) {
      SetTokenAndBufferSub(interrupt_on_complete, data_toggle, pid,
                           total_bytes);
   ...
   private:
    void SetTokenAndBufferSub(bool interrupt_on_complete, bool data_toggle,
                              UsbCtrl::PacketIdentification pid,
                              int total_bytes) {
      _token = 0;
  ...
  class QueueHead32 {
   public:
    ...
    void SetHorizontalNext(/* nullptr */) { SetHorizontalNextSub(0, 0, true); }
    ...
    void SetNextTd(/* nullptr */) { _next_td = 1; }
    bool IsNextTdNull() { return (_next_td & 1) != 0; }
    void SetCharacteristics(int max_packetsize, UsbCtrl::TransferType ttype,
                            bool head, bool data_toggle_control,
                            uint8_t endpoint_num, bool inactivate,
    ...
    void SetCapabilities(uint8_t mult, uint8_t port_number, uint8_t hub_addr,
   ...
   private:
    void SetHorizontalNext(uint32_t next) { _horizontal_next = next; }
  ...
  class DevEhciUsbController : public DevUsbController {
   public:
    ...
    virtual bool SendControlTransfer(UsbCtrl::DeviceRequest *request,
                                     virt_addr data, size_t data_size,
                                     int device_addr) override {
      return _dev_ehci->SendControlTransfer(request, data, data_size,
                                            device_addr);
    }
    virtual sptr<DevUsbController::Manager> SetupInterruptTransfer(
        uint8_t endpt_address, int device_addr, int interval,
  ...
  class DevEhciSub : public DevEhciSubBase {
   ...
   private:
     ...
     public:
      EhciManager(int num_td, QueueHead *qh,
          ...
          : _num_td(num_td),
            ...
            _container_array(new TdContainer[num_td]),
            _interrupt_qh(qh),
            _func(func),
            _master(master) {}
      virtual ~EhciManager() {
      ...
      void CopyInfo(TransferDescriptor **td_array,
                    TdContainer *container_array) {
    ...
    RingBuffer<TransferDescriptor *,
               PagingCtrl::kPageSize / sizeof(TransferDescriptor)>
        _td_buf;
  ...
  volatile uint16_t ReadCapabilityRegHciVersion() {
    return *(
        reinterpret_cast<volatile uint16_t *>(_capability_reg_base_addr + 0x2));
  }
  volatile uint32_t ReadCapabilityRegHcsParams() {
    return *(
        reinterpret_cast<volatile uint32_t *>(_capability_reg_base_addr + 0x4));
  }
  volatile uint32_t ReadCapabilityRegHccParams() {
    return *(
        reinterpret_cast<volatile uint32_t *>(_capability_reg_base_addr + 0x8));
  }
  volatile uint64_t ReadCapabilityRegHcspPortRoute() {
    return *(
        reinterpret_cast<volatile uint64_t *>(_capability_reg_base_addr + 0xC));
  }
  ...
  bool SendControlTransfer(UsbCtrl::DeviceRequest *request, virt_addr data,
                           size_t data_size, int device_addr) {
  ...
  void DisablePort(int port) {
    _op_reg_base_addr[kOpRegOffsetPortScBase + port] &=
        ~kOpRegPortScFlagPortEnable;
  }
  void ResetPort(int port) {
    DisablePort(port);
    _op_reg_base_addr[kOpRegOffsetPortScBase + port] |=
        kOpRegPortScFlagPortReset;
    ...
    while ((_op_reg_base_addr[kOpRegOffsetPortScBase + port] &
            kOpRegPortScFlagPortEnable) == 0) {
      asm volatile("" ::: "memory");
    }
  }
  ...
  void CheckQueuedTdIfCompleted(void *) {
    ...
    // TODO check not only TDs but also all interrupt sources before enable
    // interrupt
