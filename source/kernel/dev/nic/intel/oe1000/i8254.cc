...
void DevGbeI8254::SetupRx() {
  ...
  for(int i = 0; i < kRxdescNumber; i++) {
    ...
    virt_addr tmp = kernel_virtmem_ctrl->KernelHeapAlloc(kBufSize);
...
void DevGbeI8254::SetupTx() {
  ...
  // set base address of ring buffer
  virt_addr tx_desc_buf_addr = ((kernel_virtmem_ctrl->KernelHeapAlloc(sizeof(E1000TxDesc) * kTxdescNumber + 15) + 15) / 16) * 16;
  ...
  for(int i = 0; i < kTxdescNumber; i++) {
    ...
    txdesc->bufAddr = k2p(kernel_virtmem_ctrl->KernelHeapAlloc(kBufSize));
    txdesc->special = 0;
    txdesc->css = 0;
    txdesc->sta = 0;
    txdesc->cmd = 0;
