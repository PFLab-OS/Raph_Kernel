...
bool oE1000::InitPCI(uint16_t vid, uint16_t did, uint8_t bus, uint8_t device, bool mf) {
  if (vid == kVendorId) {
    ...
    case kI8254x:
      {
        DevGbeI8254 *addr = reinterpret_cast<DevGbeI8254 *>(system_memory_space->GetKernelVirtmemCtrl()->Alloc(sizeof(DevGbeI8254)));
        e1000 = new(addr) DevGbeI8254(bus, device, mf);
      }
      break;
    case kI8257x:
      {
        DevGbeI8257 *addr = reinterpret_cast<DevGbeI8257 *>(system_memory_space->GetKernelVirtmemCtrl()->Alloc(sizeof(DevGbeI8257)));
        e1000 = new(addr) DevGbeI8257(bus, device, mf);
      }
      break;
    case 0x153A:
      {
        DevGbeIch8 *addr = reinterpret_cast<DevGbeIch8 *>(system_memory_space->GetKernelVirtmemCtrl()->Alloc(sizeof(DevGbeIch8)));
...
int32_t oE1000::Receive(uint8_t *buffer, uint32_t size) {
  ...
  if(rx_available > 0) {
    ...
    gtty->Printf("e1000 rx dump; (%d ~ %d) 0x%x\n",
		  rdh, rdt, reinterpret_cast<uint64_t>(rxdesc->bufAddr));
    for(uint32_t i = 0; i < length; i++) {
	  if(buffer[i] < 0x10) gtty->Printf("%d", 0);
	  gtty->Printf("%x", buffer[i]);
	  if((i+1) % 16 == 0) gtty->Printf("\n");
	  else if((i+1) % 16 == 8) gtty->Printf(":");
	  else                gtty->Printf(" ");
	}
	gtty->Printf("\n");
