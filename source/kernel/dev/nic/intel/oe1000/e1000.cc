...
bool oE1000::InitPCI(uint16_t vid, uint16_t did, uint8_t bus, uint8_t device, bool mf) {
  if (vid == kVendorId) {
    ...
    case kI8254x:
      {
        DevGbeI8254 *addr = reinterpret_cast<DevGbeI8254 *>(kernel_virtmem_ctrl->KernelHeapAlloc(sizeof(DevGbeI8254)));
        e1000 = new(addr) DevGbeI8254(bus, device, mf);
      }
      break;
    case kI8257x:
      {
        DevGbeI8257 *addr = reinterpret_cast<DevGbeI8257 *>(kernel_virtmem_ctrl->KernelHeapAlloc(sizeof(DevGbeI8257)));
        e1000 = new(addr) DevGbeI8257(bus, device, mf);
      }
      break;
    case 0x153A:
      {
        DevGbeIch8 *addr = reinterpret_cast<DevGbeIch8 *>(kernel_virtmem_ctrl->KernelHeapAlloc(sizeof(DevGbeIch8)));
