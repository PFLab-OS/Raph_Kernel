  ...
  for(int i = 0; i < _ehdr->e_phnum; i++){
    ...
    switch(phdr->p_type){
      case PT_LOAD:
        ...
        memcpy(_membuffer + phdr->p_vaddr, &_head[phdr->p_offset], phdr->p_filesz);
        break;
    }
  }
  return ErrorState::kOk;
...
BinObjectInterface::ErrorState RaphineRing0AppObject::Init() {
  ...
  for(int i = 0; i < _ehdr->e_shnum; i++){
    const Elf64_Shdr *shdr = (const Elf64_Shdr *)(_head + _ehdr->e_shoff + _ehdr->e_shentsize * i);
    const char *sname = strtab + shdr->sh_name;
    if (strcmp(sname, ".raphine_ring0_app_info") == 0 && shdr->sh_size == kInfoSize) {
...
BinObjectInterface::ErrorState RaphineRing0AppObject::LoadMemory(bool page_mapping) {
  // 0~1GBをマップする
  _loader.Map1GAddr(0);
