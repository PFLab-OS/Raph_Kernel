...
void KernelStackCtrl::Init() {
  ...
  kassert(!system_memory_space->IsVirtAddrMapped(
      reinterpret_cast<virt_addr>(&kKernelEndAddr) -
      (PagingCtrl::kPageSize * 0)));
  kassert(system_memory_space->IsVirtAddrMapped(
      reinterpret_cast<virt_addr>(&kKernelEndAddr) -
      (PagingCtrl::kPageSize * 1)));
  kassert(!system_memory_space->IsVirtAddrMapped(
      reinterpret_cast<virt_addr>(&kKernelEndAddr) -
      (PagingCtrl::kPageSize * 2)));
  ...
  if (PagingCtrl::kPageSize < kStackSize) {
    ...
    kassert(system_memory_space->MapPhysAddrToVirtAddr(
        reinterpret_cast<virt_addr>(&kKernelEndAddr) -
            (kStackSize + PagingCtrl::kPageSize) + 1,
        paddr, size, PDE_WRITE_BIT | PDE_USER_BIT,
...
void KernelStackCtrl::InitFirstStack() {
  Locker locker(_lock);
  _stack_area_top = reinterpret_cast<virt_addr>(&kKernelEndAddr) + 1 -
                    (kStackSize + PagingCtrl::kPageSize * 2);
}
...
virt_addr KernelStackCtrl::AllocThreadStack(CpuId cpuid) {
  ...
  if (!_freed.Pop(fac)) {
    ...
    kassert(system_memory_space->MapPhysAddrToVirtAddr(
        _stack_area_top + PagingCtrl::kPageSize, paddr, kStackSize,
        PDE_WRITE_BIT, PTE_WRITE_BIT | PTE_GLOBAL_BIT | PTE_USER_BIT));
    bzero(reinterpret_cast<void *>(_stack_area_top + PagingCtrl::kPageSize),
