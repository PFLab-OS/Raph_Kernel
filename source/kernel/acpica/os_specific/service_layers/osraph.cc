...
#include "acdebug.h"
  ...
  void AcpiOsSleep(UINT64 Milliseconds) {
    timer->BusyUwait(Milliseconds * 1000);
  }

  void AcpiOsStall(UINT32 Microseconds) {
    timer->BusyUwait(Microseconds);
  }

  ACPI_STATUS AcpiOsCreateMutex(ACPI_MUTEX *OutHandle) {
    SpinLock *lock = virtmem_ctrl->New<SpinLock>();
  ...
  void AcpiOsVprintf(const char *Fmt, va_list Args) {
    while(*Fmt != '\0') {
      ...
      case '%': {
        ...
        case 'c': {
          gtty->Printf("c", static_cast<char>(va_arg(Args, int)));
          break;
        }
        case 's': {
          if (accuracy == 0) {
            gtty->Printf("s", reinterpret_cast<const char *>(va_arg(Args, const char *)));
          } else {
        ...
        case 'u':
        case 'd': {
          gtty->Printf("d", reinterpret_cast<int>(va_arg(Args, int)));
          break;
        }
        case 'p':
        case 'X':
        case 'x': {
          gtty->Printf("x", reinterpret_cast<int>(va_arg(Args, int)));
          break;
        }
        default: {
          gtty->Printf("c",'%');
          gtty->Printf("c",reinterpret_cast<char>(*Fmt));
        }
        }
        break;
      }
