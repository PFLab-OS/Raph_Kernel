...
class MultibootCtrl {
 public:
  MultibootCtrl() {
    // because multiboot_info is defined on boot/boot.S, it is placed at lower
    // memory
    extern uint32_t multiboot_info;
    extern int kLinearAddrOffset;
    _multiboot_info = reinterpret_cast<uint32_t *>(
        reinterpret_cast<virt_addr>(&multiboot_info) +
        reinterpret_cast<virt_addr>(&kLinearAddrOffset));
  ...
  phys_addr GetPhysMemoryEnd() { return _phys_memory_end; }
